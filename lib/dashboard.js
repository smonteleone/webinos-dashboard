...
function getApplauncher(){
    if (webinosApplauncher == null){
        ...
        if (!applauncherAPI || applauncherAPI.length == 0){
...
var setModule = function(name, title, path, showNavigation){
    if (typeof showNavigation === "undefined") showNavigation = true;
    if (dashboardModules.hasOwnProperty(name)) return false;
...
var getModule = function(name){
    return dashboardModules[name] || null;
};
var listModules = function() {
    var list = [];
    for (var i in dashboardModules){
        list.push({
            name: dashboardModules[i].name,
            title: dashboardModules[i].title,
            showNavigation: dashboardModules[i].showNavigation
        });
    }
    return list;
};
